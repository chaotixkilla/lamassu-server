...
module.exports = function (app) {
  ...
  app.post('/api/createuser', function (req, res, next) {
    ...
    const useFIDO = req.body.useFIDO

    users.getByName(username)
      .then(user => {
        if (user) return res.status(200).json({ message: 'User already exists!' })

        users.createUserRegistrationToken(username, role, useFIDO).then(token => {
          return res.status(200).json({ token })
        })
      })
      .catch(err => {
  ...
  app.get('/api/register', function (req, res, next) {
    ...
    users.validateUserRegistrationToken(token)
      .then(r => {
        ...
        return res.status(200).json({ username: r.username, role: r.role, useFIDO: r.useFIDO })
      })
      .catch(err => {
        console.log(err)
        res.sendStatus(400)
