  ...
  function pollQueries (serialNumber, deviceTime, deviceRec, machineVersion, machineModel) {
    ...
    return Promise.all(promises)
      .then(arr => {
        ...
        const batchableCoinsRes = arr.slice(3, cryptoCodesCount + 3)
        const batchableCoins = batchableCoinsRes.map(it => ({ batchable: it }))
        const tickers = arr.slice(cryptoCodesCount + 3, 2 * cryptoCodesCount + 3)
        const balances = arr.slice(2 * cryptoCodesCount + 3, 3 * cryptoCodesCount + 3)
        const testNets = arr.slice(3 * cryptoCodesCount + 3, arr.length - 1)
