...
function getByName (username) {
  const sql = `select id, username, hardware_credentials, role, last_accessed from users where username=$1 limit 1`
...
function findById (id) {
  const sql = 'select id, username, hardware_credentials, role, last_accessed from users where id=$1'
...
function createUserRegistrationToken (username, role, useFIDO) {
  const token = crypto.randomBytes(32).toString('hex')
  const sql = `insert into user_register_tokens (token, username, role, use_fido) values ($1, $2, $3, $4) on conflict (username)
    do update set token=$1, expire=now() + interval '30 minutes' returning *`

  return db.one(sql, [token, username, role, useFIDO])
}

function validateUserRegistrationToken (token) {
  const sql = `delete from user_register_tokens where token=$1
    returning username, role, use_fido, now() < expire as success`

  return db.one(sql, [token])
    .then(res => ({ username: res.username, role: res.role, useFIDO: res.use_fido, success: res.success }))
}
...
function initializeHardwareCredentials (id, challenge) {
  const init = {
    currentChallenge: challenge,
    devices: []
  }
  const sql = `update users set hardware_credentials=$1 where id=$2`
  return db.none(sql, [JSON.stringify(init), id])
}

function updateHardwareCredentialChallenge (id, newChallenge) {
  const sql = `update users set hardware_credentials=jsonb_set(hardware_credentials, '{currentChallenge}', $1) where id=$2`
  return db.none(sql, [JSON.stringify(newChallenge), id])
}

function getCurrentChallenge (id) {
  const sql = `select hardware_credentials->>'currentChallenge' as challenge from users where id=$1`
  return db.oneOrNone(sql, [id])
}

function getHardwareDevices (id) {
  const sql = `select hardware_credentials->>'devices' as devices from users where id=$1`
  return db.oneOrNone(sql, [id])
}

function saveHardwareDevices (id, devices) {
  const sql = `update users set hardware_credentials=jsonb_set(hardware_credentials, '{devices}', $1) where id=$2`
  return db.none(sql, [JSON.stringify(devices), id])
}
...
module.exports = {
  ...
  changeUserRole,
  toggleUserEnable,
  initializeHardwareCredentials,
  updateHardwareCredentialChallenge,
  getCurrentChallenge,
  getHardwareDevices,
  saveHardwareDevices
}
